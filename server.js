...
// --- Page Routes ---
function findFile(filename, res) {
  const locations = [
    path.join(__dirname, 'public', 'static', filename),
    path.join(__dirname, 'public', filename),
    path.join(__dirname, filename),
  ];
  for (const loc of locations) {
    if (require('fs').existsSync(loc)) return res.sendFile(loc);
  }
  res.status(404).send('File not found: ' + filename);
}
// Card page
app.get('/card/:slug', (req, res) => {
  findFile('card.html', res);
});
// Admin panel
app.get('/admin', (req, res) => {
  findFile('admin.html', res);
});
// Root
app.get('/', (req, res) => {
  res.redirect('/admin');
